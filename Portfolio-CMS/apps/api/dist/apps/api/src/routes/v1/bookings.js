...
bookingsV1Router.get('/', asyncHandler(async (req, res) => {
...
bookingsV1Router.get('/:id', asyncHandler(async (req, res) => {
...
bookingsV1Router.patch('/:id/status', asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
        return res.status(400).json({ message: 'Booking id must be numeric' });
    }
