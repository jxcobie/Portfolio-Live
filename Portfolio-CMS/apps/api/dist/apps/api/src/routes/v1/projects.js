...
projectsV1Router.get('/', asyncHandler(async (req, res) => {
...
projectsV1Router.get('/public', asyncHandler(async (_req, res) => {
    const result = await listProjects({ page: 1, pageSize: 50, status: 'published' });
    res.json(result);
}));
projectsV1Router.get('/:id', asyncHandler(async (req, res) => {
...
projectsV1Router.get('/slug/:slug', asyncHandler(async (req, res) => {
