...
messagesV1Router.get('/', asyncHandler(async (req, res) => {
...
messagesV1Router.get('/:id', asyncHandler(async (req, res) => {
...
messagesV1Router.patch('/:id/read', asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    if (Number.isNaN(id)) {
        return res.status(400).json({ message: 'Message id must be numeric' });
    }
