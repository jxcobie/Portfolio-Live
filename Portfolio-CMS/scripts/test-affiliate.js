...
async function testAffiliateSystem() {
  ...
  try {
    ...
    console.log('   Overall Stats:', JSON.stringify(dashboardRes.data.overall, null, 2));
    console.log('   Top Links:', dashboardRes.data.topLinks.length);
    console.log('   Timeline Entries:', dashboardRes.data.timeline.length);
    console.log('   Device Breakdown:', JSON.stringify(dashboardRes.data.deviceBreakdown, null, 2));
    console.log('   Category Performance:', JSON.stringify(dashboardRes.data.categoryPerformance, null, 2));
    console.log('');

    // 6. Get click details
    console.log('6️⃣  Getting click details...');
    const clicksRes = await request(`${BASE_URL}/api/affiliate/links/1/clicks`, {
      headers: { Cookie: sessionCookie },
    });
    console.log(`✅ Click Details (${clicksRes.data.meta.total} total):`);
    if (clicksRes.data.data.length > 0) {
      console.log('   Latest Click:', JSON.stringify(clicksRes.data.data[0], null, 2));
    }
    console.log('');

    // 7. Record conversion
    console.log('7️⃣  Recording conversion...');
    const conversionRes = await request(`${BASE_URL}/api/affiliate/conversion`, {
      method: 'POST',
      body: {
        link_id: 1,
        click_id: 1,
    ...
    const exportRes = await request(`${BASE_URL}/api/affiliate/export?type=links`, {
      headers: { Cookie: sessionCookie },
    });
