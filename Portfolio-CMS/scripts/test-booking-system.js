...
async function testBookingSystem() {
  ...
  try {
    ...
    const availRes = await fetch(`${CMS_URL}/api/bookings/availability/${today}?duration=30`);
    const avail = await availRes.json();
    console.log(`✅ Availability: ${avail.availableSlots.length} slots available`);
    ...
    const avail2Res = await fetch(`${CMS_URL}/api/bookings/availability/${today}?duration=30`);
    const avail2 = await avail2Res.json();
    console.log(`✅ Updated availability: ${avail2.availableSlots.length} slots`);
    ...
    const statusRes = await fetch(`${CMS_URL}/api/bookings/${booking.bookingId}/status`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': API_KEY
    ...
    console.log(`   - Email notifications: ${booking.message.includes('email') ? '✅' : '⚠️'}`);
