...
async function testAffiliateLinks() {
  ...
  // Get analytics
  const analyticsRes = await request(`${BASE_URL}/api/affiliate/analytics?timeframe=7d`);
  console.log(`✅ Affiliate analytics:`);
  console.log(`   Total clicks: ${analyticsRes.data.summary.totalClicks}`);
  console.log(`   Total conversions: ${analyticsRes.data.summary.totalConversions}`);
  console.log(`   Total revenue: $${analyticsRes.data.summary.totalRevenue}`);

  // Get dashboard
  const dashboardRes = await request(`${BASE_URL}/api/affiliate/dashboard?period=7d`);
  console.log(`✅ Affiliate dashboard:`);
  console.log(`   Total links: ${dashboardRes.data.overall.total_links}`);
  console.log(`   Top performing link: ${dashboardRes.data.topLinks[0]?.name || 'None'}`);
...
async function testCampaigns() {
  ...
  try {
    ...
    if (listRes.status === 404) {
      console.log('⚠️  Campaign endpoints not yet implemented (tables created, endpoints pending)');
      console.log('   Database tables ready: campaigns, tracked_links, link_clicks, campaign_stats');
      return;
    }
...
async function testIncome() {
  ...
  try {
    ...
    if (listRes.status === 404) {
      console.log('⚠️  Income endpoints not yet implemented (tables created, endpoints pending)');
    ...
    if (dashboardRes.status === 200) {
      ...
      console.log(`   Current month: $${dashboardRes.data.currentMonth?.total || 0}`);
...
async function testVisitorAnalytics() {
  ...
  try {
    ...
    const dashboardRes = await request(`${BASE_URL}/api/analytics/dashboard?timeframe=7d`);

    if (dashboardRes.status === 404 || !dashboardRes.data.overview) {
      console.log('⚠️  Enhanced analytics dashboard not yet implemented');
      console.log('   Database tables ready: visitor_sessions, page_views, daily_stats');
      ...
      if (basicRes.status === 200) {
        ...
        console.log(`   Total events: ${basicRes.data.summary?.totalEvents || 0}`);
      ...
      console.log(`   Avg pages/session: ${overview.avg_pages_per_session || 0}`);
    ...
    if (realtimeRes.status === 200 && realtimeRes.data.active_visitors !== undefined) {
