  ...
  try {
    ...
    if (projectsRes.ok) {
      ...
      if (data.projects && data.projects.length > 0) {
        ...
        const singleRes = await fetch(
          `${CMS_URL}/api/projects/public/${firstProject.id}`,
        );
        ...
        if (singleRes.ok) {
          ...
          if (singleData.project) {
            ...
            console.log(
              `   - Has cover image: ${singleData.project.coverImage ? "Yes" : "No"}`,
            );
          } else {
    ...
    try {
      ...
      if (nextRes.ok) {
        ...
        console.log(
          `✅ Next.js API working: ${nextData.projects?.length || 0} projects`,
        );
      } else {
