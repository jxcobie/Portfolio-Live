...
const serviceApiKey = process.env.CMS_API_KEY;

if (!sessionSecret) {
  throw new Error(
    "SESSION_SECRET environment variable is required for the CMS to start.",
  );
}

if (!serviceApiKey) {
  throw new Error(
    "CMS_API_KEY environment variable is required for authenticated requests.",
  );
}
...
const sanitizeInput = (value, { multiline = false, maxLength } = {}) => {
...
const requireServiceApiKey = (req, res, next) => {
  if (req.session && req.session.authenticated) {
    return next();
  }

  const providedKey =
    req.get("x-cms-api-key") || req.get("x-api-key") || req.query.api_key;
...
const validateMessagePayload = (payload = {}) => {
  ...
  const name = sanitizeInput(payload.name, { maxLength: 100 });
  ...
  const subject = sanitizeInput(payload.subject || "Contact Form Submission", {
    maxLength: 300,
  });
...
const validateAnalyticsPayload = (payload = {}) => {
  const errors = [];

  const eventType = sanitizeInput(payload.event_type, { maxLength: 100 });
  if (!eventType) {
    errors.push({ field: "event_type", message: "Event type is required" });
  }

  const pageUrl = sanitizeInput(payload.page_url || "", { maxLength: 2000 });
  const referrer = sanitizeInput(payload.referrer || "", { maxLength: 2000 });
...
const validateBookingPayload = (payload = {}) => {
  ...
  if (!Number.isInteger(duration) || duration < 15 || duration > 240) {
    errors.push({
      field: "duration",
      message: "Duration must be an integer between 15 and 240 minutes",
    });
  }

  const meetingType = sanitizeInput(payload.meetingType || "", { maxLength: 100 });
  const phone = sanitizeInput(payload.phone || "", { maxLength: 50 });
  const notes = sanitizeInput(payload.notes || "", { multiline: true, maxLength: 2000 });
...
const createIndex = (name, sql) => {
  db.run(sql, (err) => {
    ...
    if (err.message && err.message.includes("no such table")) {
      console.warn(`Skipping index ${name}: dependent table missing.`);
      return;
    }
...
db.serialize(() => {
  ...
  const bootstrapUsername = process.env.CMS_BOOTSTRAP_ADMIN_USER;
  const bootstrapPassword = process.env.CMS_BOOTSTRAP_ADMIN_PASSWORD;

  if (bootstrapUsername && bootstrapPassword) {
    const hashedPassword = bcrypt.hashSync(bootstrapPassword, 12);
    db.run(
      `INSERT OR IGNORE INTO users (username, password, role) VALUES (?, ?, ?)`,
      [bootstrapUsername, hashedPassword, "admin"],
      (err) => {
        if (err) {
          console.error("Failed to seed bootstrap admin user:", err);
        }
      },
    );
  } else {
    console.warn(
      "⚠️  CMS bootstrap admin credentials not provided. Set CMS_BOOTSTRAP_ADMIN_USER and CMS_BOOTSTRAP_ADMIN_PASSWORD to seed an initial administrator.",
    );
  }
  ...
  db.get(`SELECT COUNT(*) as count FROM technologies`, (err, row) => {
    if (!err && row.count === 0) {
      ...
      techs.forEach((tech) => {
        db.run(`INSERT INTO technologies (name, category) VALUES (?, ?)`, [
          tech,
          "General",
        ]);
      });
...
const authRateLimitWindowMinutes = parseInt(
  process.env.RATE_LIMIT_AUTH_WINDOW_MINUTES ||
...
const authMaxRequests = parseInt(
  ...
  db.get(query, [projectId], (err, project) => {
    ...
    db.all(
      ...
      `.trim();
        ...
        const coverImage =
          images && images.find((img) => img.image_type === "cover");
        ...
        const formattedProject = {
          ...
          galleryImages: images
            ? images.filter((img) => img.image_type === "gallery")
            : [],
          technologies: project.technologies
            ? project.technologies.split(",").map((name, index) => ({
                id: index + 1,
    ...
    db.run(query, params, function (err) {
      ...
      if (uploadedFiles.length > 0) {
        uploadedFiles.forEach((file, index) => {
          const imageUrl = `/uploads/${file.filename}`;
          const imageType =
            projectData.coverImageIndex === index ? "cover" : "gallery";
...
app.post("/api/messages", requireServiceApiKey, (req, res) => {
...
// ==================== ANALYTICS ROUTES ====================
app.post("/api/analytics/track", requireServiceApiKey, (req, res) => {
  const validation = validateAnalyticsPayload(req.body);
  ...
  const { event_type, event_data, page_url, referrer } = validation.data;
  const ipAddress = req.ip;
  const userAgent = req.get("user-agent");
...
app.post("/api/campaigns", requireAuth, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      ...
      db.run("INSERT INTO campaign_stats (campaign_id) VALUES (?)", [
        this.lastID,
      ]);
...
app.get("/api/campaigns/:id", requireAuth, (req, res) => {
  db.get(
    ...
    (err, campaign) => {
      ...
      db.all(
        "SELECT * FROM tracked_links WHERE campaign_id = ?",
        [req.params.id],
        (err, links) => {
...
app.put("/api/campaigns/:id", requireAuth, (req, res) => {
  const { name, platform, comment_format, description, target_url, is_active } =
    req.body;
  db.run(
...
app.post("/api/campaigns/:id/links", requireAuth, (req, res) => {
  db.get(
    "SELECT id, target_url FROM campaigns WHERE id = ?",
    [req.params.id],
    (err, campaign) => {
      if (err || !campaign)
        return res.status(404).json({ error: "Campaign not found" });
...
app.get("/l/:shortCode", (req, res) => {
  ...
  db.get(
    ...
    (err, link) => {
      ...
      db.run(
        "UPDATE tracked_links SET total_clicks = total_clicks + 1 WHERE id = ?",
        [link.id],
      );
      db.get(
        ...
        (err, result) => {
          if (!err && result && result.count === 1) {
            db.run(
              "UPDATE tracked_links SET unique_clicks = unique_clicks + 1 WHERE id = ?",
              [link.id],
            );
            db.run(
              "UPDATE campaign_stats SET total_clicks = total_clicks + 1, unique_clicks = unique_clicks + 1, last_click = CURRENT_TIMESTAMP WHERE campaign_id = ?",
              [link.campaign_id],
            );
          } else {
            db.run(
              "UPDATE campaign_stats SET total_clicks = total_clicks + 1, last_click = CURRENT_TIMESTAMP WHERE campaign_id = ?",
              [link.campaign_id],
            );
          }
        },
...
app.get("/api/campaigns/:id/analytics", requireAuth, (req, res) => {
  ...
  let completed = 0;
  db.all(
    "SELECT DATE(lc.clicked_at) as date, COUNT(*) as clicks, COUNT(DISTINCT lc.ip_address) as unique_visitors FROM link_clicks lc JOIN tracked_links tl ON lc.link_id = tl.id WHERE tl.campaign_id = ? GROUP BY DATE(lc.clicked_at) ORDER BY date DESC",
    [req.params.id],
    (err, data) => {
  ...
  db.all(
    "SELECT lc.device_type, COUNT(*) as clicks FROM link_clicks lc JOIN tracked_links tl ON lc.link_id = tl.id WHERE tl.campaign_id = ? GROUP BY lc.device_type",
    [req.params.id],
    (err, data) => {
  ...
  db.all(
    "SELECT lc.browser, COUNT(*) as clicks FROM link_clicks lc JOIN tracked_links tl ON lc.link_id = tl.id WHERE tl.campaign_id = ? GROUP BY lc.browser",
    [req.params.id],
    (err, data) => {
...
app.get("/api/campaigns/compare", requireAuth, (req, res) => {
  let query =
    "SELECT c.*, cs.total_clicks, cs.unique_clicks, cs.ctr FROM campaigns c LEFT JOIN campaign_stats cs ON c.id = cs.campaign_id WHERE c.is_active = 1";
  const params = [];
...
app.get("/api/income", requireAuth, (req, res) => {
  let query =
    "SELECT ir.*, p.title as project_title FROM income_records ir LEFT JOIN projects p ON ir.project_id = p.id WHERE 1=1";
  const params = [];
  if (req.query.status) {
    query += " AND ir.status = ?";
  ...
  if (req.query.year && req.query.month) {
    query +=
      " AND strftime('%Y', ir.payment_date) = ? AND strftime('%m', ir.payment_date) = ?";
...
app.get("/api/income/dashboard", requireAuth, (req, res) => {
  ...
  let completed = 0;
  db.all(
    "SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count FROM income_records WHERE strftime('%Y-%m', payment_date) = strftime('%Y-%m', 'now') AND status = 'received'",
    [],
    (err, data) => {
  ...
  db.all(
    "SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count FROM income_records WHERE strftime('%Y', payment_date) = strftime('%Y', 'now') AND status = 'received'",
    [],
    (err, data) => {
  ...
  db.all(
    "SELECT category, COALESCE(SUM(amount), 0) as total, COUNT(*) as count FROM income_records WHERE status = 'received' GROUP BY category",
    [],
    (err, data) => {
  ...
  db.all(
    "SELECT client_name, COALESCE(SUM(amount), 0) as total, COUNT(*) as payments FROM income_records WHERE status = 'received' GROUP BY client_name ORDER BY total DESC LIMIT 10",
    [],
    (err, data) => {
  ...
  db.all(
    "SELECT id, client_name, amount, payment_date, invoice_number FROM income_records WHERE status = 'pending' ORDER BY payment_date ASC",
    [],
    (err, data) => {
...
app.get("/api/analytics/dashboard", requireAuth, (req, res) => {
  ...
  db.all(
    "SELECT referrer_source, COUNT(*) as visitors FROM visitor_sessions WHERE first_visit >= datetime('now', '-" +
      days +
      " days') GROUP BY referrer_source ORDER BY visitors DESC",
    [],
    (err, data) => {
  ...
  db.all(
    "SELECT device_type, COUNT(*) as visitors FROM visitor_sessions WHERE first_visit >= datetime('now', '-" +
      days +
      " days') GROUP BY device_type",
    [],
    (err, data) => {
      results.devices = data || [];
      if (++completed === 4) res.json(results);
  ...
  db.all(
    "SELECT page_url, COUNT(*) as views, ROUND(AVG(time_on_page), 2) as avg_time_on_page FROM page_views WHERE viewed_at >= datetime('now', '-" +
      days +
      " days') GROUP BY page_url ORDER BY views DESC LIMIT 10",
    [],
    (err, data) => {
      results.pages = data || [];
      if (++completed === 4) res.json(results);
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to create affiliate link" });
      }
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to update affiliate link" });
      }
...
app.delete("/api/affiliate/links/:id", requireApiKey, async (req, res) => {
  ...
  db.run("DELETE FROM affiliate_clicks WHERE link_id = ?", [id], (err) => {
    ...
    db.run(
      "DELETE FROM affiliate_performance WHERE link_id = ?",
      [id],
      (err) => {
        if (err) {
        ...
        db.run(
          ...
          function (err) {
            if (err) {
              ...
              return res
                .status(500)
                .json({ error: "Failed to delete affiliate link" });
            }

            if (this.changes === 0) {
              return res
                .status(404)
                .json({ error: "Affiliate link not found" });
            }
...
app.get("/go/:shortCode", async (req, res) => {
  ...
  db.get(
    ...
    (err, link) => {
      ...
      db.run(
        clickQuery,
        [link.id, ipAddress, userAgent, referrer, deviceType],
        function (err) {
          if (err) {
  ...
  db.all(performanceQuery, params, (err, performance) => {
    ...
    db.all(summaryQuery, summaryParams, (err, links) => {
      ...
      const summary = {
        ...
        totalConversions: links.reduce(
          (sum, link) => sum + (link.conversions || 0),
          0,
        ),
        totalRevenue: links.reduce(
          (sum, link) => sum + (parseFloat(link.revenue) || 0),
          0,
        ),
...
app.get("/api/affiliate/links/:id/clicks", requireApiKey, async (req, res) => {
  ...
  db.all(query, [id, parseInt(limit), parseInt(offset)], (err, clicks) => {
    ...
    // Get total count
    db.get(
      "SELECT COUNT(*) as total FROM affiliate_clicks WHERE link_id = ?",
      [id],
      (err, row) => {
        if (err) {
          console.error("Database error:", err);
      ...
      db.all(clicksTimelineQuery, (err, timeline) => {
        ...
        db.all(deviceBreakdownQuery, (err, devices) => {
          ...
          db.all(categoryPerformanceQuery, (err, categories) => {
            ...
            results.overall.ctr =
              results.overall.total_clicks > 0
                ? (
                    (results.overall.unique_clicks /
                      results.overall.total_clicks) *
                    100
                  ).toFixed(2)
                : 0;
...
app.get("/api/affiliate/export", requireApiKey, (req, res) => {
  ...
  db.all(query, (err, rows) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="affiliate-${type}-${Date.now()}.csv"`,
    );
    res.send(csv);
  });
});
...
// CREATE BOOKING
app.post("/api/bookings", requireServiceApiKey, async (req, res) => {
  ...
  const { name, email, phone, date, time, duration, meetingType, notes } =
    validation.data;
  ...
  db.get(
    ...
    async (err, existing) => {
      if (err) {
        console.error("Failed to validate booking slot:", err);
        return res.status(500).json({ error: "Database error" });
      }
      if (existing) {
      ...
      db.run(
        ...
        async function (err) {
          ...
          db.run("UPDATE bookings SET meeting_link = ? WHERE id = ?", [
            meetingLink,
            bookingId,
          ]);
          ...
          db.get(
            ...
            async (err, booking) => {
              if (err) {
                console.error("Failed to load booking for confirmation:", err);
                return res.status(500).json({
                  error: "Booking created but failed to load confirmation",
                });
              }
              ...
              try {
                ...
                console.log(
                  `✅ Booking created: ${booking.name} - ${booking.date} ${booking.time}`,
                );
              } catch (emailError) {
...
app.get(
  ...
  (req, res) => {
    const requestedDate = sanitizeInput(req.params.date, { maxLength: 10 });
    ...
    const durationInput = Number(req.query.duration ?? 30);
    if (
    ...
    const normalizedDuration = durationInput;
    ...
    db.get(
      ...
      (err, override) => {
        if (err) {
          console.error("Failed to load availability override:", err);
          return res.status(500).json({ error: "Database error" });
        }
        ...
        db.all(
          ...
          (err, workingHours) => {
            ...
            db.all(
              "SELECT * FROM available_slots WHERE date = ?",
              [requestedDate],
              (err, customSlots) => {
                if (err) {
                  ...
                  (err, blockedSlots) => {
                    ...
                    db.all(
                      ...
                      (err, bookedSlots) => {
                        ...
                        const timeRanges =
                          customSlots.length > 0 ? customSlots : workingHours;
                        const allSlots = [];
                        ...
                        timeRanges.forEach((range) => {
                          const [startHour, startMinute] = range.start_time
                            .split(":")
                            .map((v) => parseInt(v, 10));
                          const [endHour, endMinute] = range.end_time
                            .split(":")
                            .map((v) => parseInt(v, 10));
                          ...
                          for (
                            ...
                            if (minute + normalizedDuration <= endMin) {
                        ...
                        const availableSlots = allSlots.filter((slot) => {
                          ...
                          const isBlocked = blockedSlots.some((blocked) => {
                            return (
                              slot >= blocked.start_time &&
                              slot < blocked.end_time
                            );
                          });
                        ...
                        res.json({
                          ...
                          workingHours: timeRanges.map((range) => ({
...
app.post("/api/bookings/availability/override", requireAuth, (req, res) => {
  ...
  if (!date || is_available === undefined) {
    return res
      .status(400)
      .json({ error: "Date and is_available are required" });
  }
...
app.post("/api/bookings/block", requireAuth, (req, res) => {
  ...
  if (!date || !start_time || !end_time) {
    return res
      .status(400)
      .json({ error: "Date, start_time, and end_time required" });
  }
...
app.delete("/api/bookings/available/:id", requireAuth, (req, res) => {
  db.run(
    "DELETE FROM available_slots WHERE id = ?",
    [req.params.id],
    function (err) {
      if (err) return res.status(500).json({ error: "Failed to delete" });
      res.json({ success: true });
...
app.use((err, req, res, next) => {
  if (err && err.message === "Not allowed by CORS") {
    return res
      .status(403)
      .json({ error: "Origin not allowed by CORS policy." });
  }
